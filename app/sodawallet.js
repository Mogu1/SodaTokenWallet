...
function setFiatBalanceText(balanceSodaToken, fiatCurrencySymbol = "") {
    ...
    let API_URL = BASE_API_URL + "/sodatoken/?convert=" + fiatCurrencySymbol;

    axios.get(API_URL).then(response => {
        let resp = response.data;
        let zenPrice = parseFloat(resp[0]["price_" + fiatCurrencySymbol.toLowerCase()]);
        const now = new Date().toLocaleTimeString();
        let balance = parseFloat(balanceSodaToken) * zenPrice;
...
function showAddrDetail(addr) {
    showDialogFromTemplate("addrDialogTemplate", dialog => {
        ...
        dialog.querySelector(".addrInfoLink").addEventListener("click", () => openSodaTokenExplorer("address/" + addr));
...
function showTxDetail(txObj) {
    ...
    showDialogFromTemplate(templateId, dialog => {
        ...
        dialog.querySelector(".txInfoLink").addEventListener("click", () => openSodaTokenExplorer("tx/" + txObj.txid));
...
function setTotalBalance(balanceSodaToken) {
    setBalanceText(totalBalanceNode, balanceSodaToken);
    setFiatBalanceText(balanceSodaToken);
}
...
function initDepositView() {
    ...
    depositSaveQrcodeButton.addEventListener("click", () => {
        ...
        pdf.save(`soda-deposit-${addr}.pdf`);
    });
}
