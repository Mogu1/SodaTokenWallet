...
function showNotification(message) {
    const notif = new Notification("SodaWallet", {
        body: message,
 ...
 * @returns the `<soda-dialog>` node of the created dialog
 */
function createDialogFromTemplate(templateId) {
    const dialog = cloneTemplate(templateId);
    if (dialog.tagName !== "SODA-TOKEN-WALLER-DIALOG")
...
function showSettingsDialog() {
    showDialogFromTemplate("settingsDialogTemplate", dialog => {
        ...
        dialog.querySelector(".settingsSave").addEventListener("click", () => {
            ...
            let sodaBalance = getsodaBalance();
            setFiatBalanceText(sodaBalance, inputFiatCurrency.value);
...
function openSodaTokenExplorer(path) {
    openUrl(settings.explorerUrl + "/" + path);
}

function getsodaBalance() {
