...
const defaultSettings = {
    ...
    apiUrls: [
        "https://explorer.sodatoken.org/insight-api-soda",
        "http://explorer.sodatoken.org/insight-api-soda"
    ],
    txHistory: 50,
    fiatCurrency: "USD",
    lang: "en"
...
function getRootConfigPath() {
    ...
    if (os.platform() === "win32" || os.platform() === "darwin") {
        rootPath = app.getPath("appData") + "/SodaWallet/";
    } else if (os.platform() === "linux") {
        rootPath = app.getPath("home") + "/.soda/";
...
function generateNewWallet(login, password) {
    ...
    for (i = 0; i <= 42; i += 1) {
        pk = sodatokenjs.address.WIFToPrivKey(privateKeys[i]);
        pubKey = sodatokenjs.address.privKeyToPubKey(pk, true);
        db.run("INSERT INTO wallet VALUES (?,?,?,?,?)", [null, pk, sodatokenjs.address.pubKeyToAddr(pubKey), 0, ""]);
    }
...
function getNewAddress(name) {
    ...
    pk = sodatokenjs.address.WIFToPrivKey(privateKeys[0]);
    addr = sodatokenjs.address.pubKeyToAddr(sodatokenjs.address.privKeyToPubKey(pk, true));
...
function exportWalletSodaWallet(ext, encrypt) {
...
function importWalletSodaWallet(ext, encrypted) {
            ...
            submenu: [
                {
                    ...
                    click() {
                        exportWalletSodaWallet("awd", true);
                    }
                },
                {
                    label: tr("menu.backupUnencrypted", "Backup UNENCRYPTED wallet"),
                    click() {
                        exportWalletSodaWallet("uawd", false);
                    }
                },
                { type: "separator" },
                {
                    label: tr("menu.importEncrypted", "Import ENCRYPTED SodaWallet wallet"),
                    click() {
                        importWalletSodaWallet("awd", true);
                    }
                },
                {
                    label: tr("menu.importUnencrypted", "Import UNENCRYPTED SodaWallet wallet"),
                    click() {
                        importWalletSodaWallet("uawd", false);
                    }
                },
                { type: "separator" },
                {
...
function updateBlockchainView(webContents) {
    ...
    fetchBlockchainChanges(addrObjs, knownTxIds).then(result => {
        ...
        // Why here ? In case balance is unchanged the 'update-wallet-balance' is never sent, but the SodaToken/Fiat balance will change.
...
function exportPKs() {
    ...
    dialog.showSaveDialog({
        ...
        defaultPath: "soda-private-keys.txt"
    }, filename => {
        if (filename)
    ...
    function importFromFile(filename) {
        ...
        fs.readFileSync(filename).toString().split('\n').filter(x => x).forEach(line => {
            ...
            if (!matches)
                ...
                const pk = matches[0];
                try {
                    const pub = sodatokenjs.address.privKeyToPubKey(pk, true);
                    const addr = sodatokenjs.address.pubKeyToAddr(pub);
...
ipcMain.on("send", function (event, fromAddress, toAddress, fee, amount){
    ...
    if (paramErrors.length) {
        ...
        let SodaTokenApi = settings.apiUrls[0];
        if (!SodaTokenApi) {
            console.log( tr("wallet.tabWithdraw.messages.SodaTokenApi","No SodaToken api in settings"));
            return;
        }
        if ((SodaTokenApi.substr(SodaTokenApi.length - 1)) === "/"){
            SodaTokenApi = SodaTokenApi.substr(0, SodaTokenApi.length - 1);
        }

        const prevTxURL = SodaTokenApi + "/addr/" + fromAddress + "/utxo";
        const infoURL = SodaTokenApi + "/status?q=getInfo";
        const sendRawTxURL = SodaTokenApi + "/tx/send";

        // Building our transaction TXOBJ
        // Calculate maximum SODA TOKEN satoshis that we have
        let satoshisSoFar = 0;
        let history = [];
            ...
            if (tx_err) {
                ...
                request.get(infoURL, function (info_err, info_resp, info_body) {
                    if (info_err) {
                        ...
                        const blockHashURL = SodaTokenApi + "/block-index/" + blockHeight;
                            ...
                            if (bhash_err) {
                                ...
                                if (satoshisSoFar < amountInSatoshi + feeInSatoshi) {
                                    ...
                                    /*let errStr = "You don't have so many funds! You wanted to send: " +
                                        Number((amountInSatoshi + feeInSatoshi) / 100000000).toFixed(8) + " SODA TOKEN, but your balance is only: " +
                                        Number(satoshisSoFar / 100000000).toFixed(8) + " SODA TOKEN.";*/
                                    console.log(errStr);
                                    ...
                                    // Create transaction
                                    let txObj = sodatokenjs.transaction.createRawTx(history, recipients, blockHeight, blockHash);

                                    // Sign each history transcation
                                    for (let i = 0; i < history.length; i ++) {
                                        txObj = sodatokenjs.transaction.signTx(txObj, i, privateKey, true)
                                    }

                                    // Convert it to hex string
                                    const txHexString = sodatokenjs.transaction.serializeTx(txObj);
...
/**
 * @param event
 * @param {array} fromAddresses - Array of strings, array of SODA TOKEN addresses
 * @param toAddress - one destination SODA TOKEN address
...
ipcMain.on("send-many", function (event, fromAddresses, toAddress, fee, thresholdLimit = 42.0) {
    ...
    if (paramErrors.length) {
        ...
        // CHECK SODA TOKEN API -----------------------------------------------------------------------------------------------
        let SodaTokenApi = settings.apiUrls[0];
        if (!SodaTokenApi) {
            err = tr("wallet.tabWithdraw.messages.SodaTokenApi", "SodaToken API is not set in settings!");
            console.log(err);
        ...
        // GET PREVIOUS TRANSACTIONS -----------------------------------------------------------------------------------
        let prevTxURL = SodaTokenApi + "/addrs/";
        for (let i = 0; i < nFromAddresses; i++) {
                            ...
                            if (bhash_err) {
                                ...
                                if ((satoshisSoFar - (nFromAddresses * thresholdLimitInSatoshi)) < feeInSatoshi) {
                                    ...
                                    for (let i = 0; i < history.length; i++) {
                                        ...
                                        txObj = sodatokenjs.transaction.signTx(txObj, i, privateKeys[index], true)
                                    }
...
//         }
//         pubKey = sodatokenjs.address.privKeyToPubKey(pk, true);
//         db.run("INSERT OR IGNORE INTO wallet VALUES (?,?,?,?,?)", [null, pk, sodatokenjs.address.pubKeyToAddr(pubKey), 0, ""]);
//     }
//
//     let data = db.export();
